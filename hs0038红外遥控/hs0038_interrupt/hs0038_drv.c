...
#if 0
//精准延时
static void hs0038_delay_us(int us)
{
    int     T1, T2;
...
static irqreturn_t hs0038_isr(int irq, void * dev_id)
{
    ...
    if(!ret){
		...
		dev_info(g_dev,"hs0038 get data = 0x%x \n",val);
    }
	else if(ret ==-2){		
		hs0038_edge_irq_cnt = 0;		
...
static int hs0038_probe(struct platform_device * pdev)
{
    ...
    if(devm_request_irq(g_dev, irq, hs0038_isr, IRQF_TRIGGER_FALLING|IRQF_TRIGGER_RISING, "hs0038", NULL)){
