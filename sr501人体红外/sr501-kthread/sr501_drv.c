...
static ssize_t sr501_drv_read(struct file * file, char __user *buf, size_t size, loff_t * offset)
{
    ...
    //1.有数据就copy to user
    //2.无数据就休眠,放入等待队列
    wait_event_interruptible(sr501_waitqueue, sr501_data);
...
static int sr501_kthreadfn(void * data)
{
    printk("%s %s line %d\n", __FILE__, __FUNCTION__, __LINE__);
    ...
    while (!kthread_should_stop()) {
		...
		set_current_state(TASK_INTERRUPTIBLE);//必须有
		schedule_timeout(HZ*1);//HZ ==> 1s
    }
	return 0;
}
...
static int sr501_probe(struct platform_device * pdev)
{
    ...
    //3.kthread监测gpio pin
    sr501_kthread    = kthread_run(sr501_kthreadfn, NULL, "sr501d");
    if (IS_ERR(sr501_kthread))
        return PTR_ERR(sr501_kthread);
